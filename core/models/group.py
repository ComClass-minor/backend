...
class Group(BaseModel):
    ...
    number_of_coadmins: Optional[int] = Field(default=max_limit//10)
    number_of_elders: Optional[int] = Field(default=max_limit//5)
    ...
    async def create_group(cls, group: 'Group') -> 'Group':
        try:
            creator = await Student.get_student(group['creator_id'])
            assertions.assert_true(creator, "Creator not found")
        ...
        try:
            ...
            creator.group_list.append({str(group.inserted_id) : "Admin"})
            return group
        except Exception as e:
            logging.error(f"An error occurred: {e}")
            return None
